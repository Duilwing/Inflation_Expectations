...
SCE_micro <- SCE_micro %>%
 ...
 ~ 0,Expectation-last_exp!=0 ~ 1))%>%
  mutate("Updating Difference"=Expectation-last_exp)%>%
  ungroup(userid)
...
-plotting distribution of updating frequency
...
ggplot(data=SCE_micro_frequency, aes(x=`Updating Frequency`)) + geom_density(adjust=1, alpha=0.8,color="#440154FF", fill="#440154FF") + labs(x="Updating Frequency in Months",y="Distribution") +scale_x_continuous(breaks = seq(0, 12, by = 1))+ theme_minimal()
