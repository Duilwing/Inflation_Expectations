...
SCE_Data_sum <-bind_cols(SCE_Data, SCE_Data_unc[2],SCE_Data_demo[2:7],CPI[2])%>%
  rename("Education High School or Less"=`Education High School or Less`,"College"=`Education Some College`,"BA or Higher"=`Education BA or Higher`, CPI=CPIAUCSL_PC1)
...
#pivot longer
SCE_plot <- select(SCE_Data_sum,c(1:5))%>%
...
CE_Data_01 <- SCE_Data_sum%>%
  filter(Date<"2020-05-01")

CE_Data_02 <- SCE_Data_sum%>%
...
SCE_Data_ed <- SCE_Data_sum%>%
  select(c(1,10:12))%>%
  pivot_longer(cols=c(2:4), names_to = "Education Levels", values_to = "Expectations", values_drop_na = FALSE)

#Rang of Education Levels
SCE_Data_ed$`Education Levels` <- factor(SCE_Data_ed$`Education Levels`, levels = c("Education High School or Less","College","BA or Higher"))
...
infl_sum <- bind_cols(infl_US,infl_r_US)%>%
  select(c(1,2,4,))%>%
...
ggplot(data=infl_sum) + aes(x=Date, y =`Relative Rankings`, color =`Searched Terms`) + geom_line(size=0.9,alpha=1)+scale_color_manual(values=c("#00798c","#B9314F"))+labs(x="Years")+scale_x_date(date_labels = "%Y",date_breaks = "1 year")+ theme_minimal()  
...
- we use the idea of dräger (ZITAT)
